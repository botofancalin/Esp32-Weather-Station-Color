...
bool WeatherStationClass::GetParams(fs::FS &fs, const char *path)
{
    ...
    if (sdfile.available())
    {
        this->WeatherParams = sdfile.readStringUntil(EOF);
        this->WUNDERGROUND_CITY = GetTextData(&WeatherParams, String("CITY="));
        this->WUNDERGROUND_COUNTRY = GetTextData(&WeatherParams, String("COUNTRY="));
        this->WUNDERGRROUND_LANGUAGE = GetTextData(&WeatherParams, String("LANGUAGE="));
        this->WUNDERGRROUND_API_KEY = GetTextData(&WeatherParams, String("APIKEY="));
        this->Utc_Offset = atoi((GetTextData(&WeatherParams, String("TIME_OFFSET="))).c_str());
    }
...
void WeatherStationClass::updateData(bool msg = false)
{
    ...
    if (msg)
    {
        ...
        M5.Lcd.drawCentreString("Reading Weather Info...", 160, 120, 1);
    }
...
void WeatherStationClass::drawTime()
{
    ...
    M5.Lcd.drawRightString(time_str, 315, 5, 1);
    ...
    if (!dateDrawn || (timeinfo->tm_min == 0 && timeinfo->tm_sec == 1))
    {
        ...
        M5.Lcd.drawString(oldDate, 5, 18, 1);
        M5.Lcd.setTextColor(WHITE);
        M5.Lcd.setFreeFont(&FreeSans9pt7b);
        M5.Lcd.drawString(date, 5, 18, 1);
        oldDate = date;
        dateDrawn = true;
    }
...
void WeatherStationClass::drawCurrentWeather()
{
    ...
    M5.Lcd.drawJpg((uint8_t *)getMeteoconIconFromProgmem(conditions.weatherIcon),
                   sizeof(getMeteoconIconFromProgmem(conditions.weatherIcon)) / getMeteoconIconFromProgmem(conditions.weatherIcon)[0], 100, 45);
    M5.Lcd.setTextColor(BLUE);
    M5.Lcd.setFreeFont(&FreeSans12pt7b);
    M5.Lcd.drawRightString(WUNDERGROUND_CITY, 315, 50, 1);
    M5.Lcd.setTextColor(WHITE);

    String temp = conditions.currentTemp + degreeSign;
    M5.Lcd.setFreeFont(&FreeSans24pt7b);
    M5.Lcd.drawRightString(temp, 315, 75, 1);
    M5.Lcd.setTextColor(ORANGE);
    M5.Lcd.setFreeFont(&FreeSans9pt7b);
    M5.Lcd.drawRightString(conditions.weatherText, 315, 117, 1);
    M5.Lcd.drawFastHLine(0, 140, 320, WHITE);
    M5.Lcd.setTextColor(CYAN);
    M5.Lcd.drawCentreString("Hum: " + conditions.humidity, 40, 50, 1);
    M5.Lcd.drawCentreString("Precip: " + forecasts[0].PoP + "%", 40, 117, 1);
    M5.Lcd.setTextColor(YELLOW);
    M5.Lcd.drawCentreString("Wind: " + conditions.windDir, 40, 75, 1);
    M5.Lcd.drawCentreString(conditions.windSpeed, 40, 95, 1);
}
...
void WeatherStationClass::drawForecastDetail(uint16_t x, uint16_t y, uint8_t dayIndex)
{
    ...
    M5.Lcd.setTextColor(YELLOW);
    M5.Lcd.setFreeFont(&FreeSans9pt7b);
    M5.Lcd.drawCentreString(day, x + 25, y - 15, 2);
    M5.Lcd.setTextColor(WHITE);
    M5.Lcd.drawCentreString(forecasts[dayIndex].forecastLowTemp + "|" + forecasts[dayIndex].forecastHighTemp, x + 25, y, 2);
    M5.Lcd.drawJpg((uint8_t *)getMiniMeteoconIconFromProgmem(forecasts[dayIndex].forecastIcon),
    ...
    M5.Lcd.setTextColor(BLUE);
    M5.Lcd.setFreeFont(&FreeSans9pt7b);
    M5.Lcd.drawCentreString(forecasts[dayIndex].PoP + "%", x + 25, y + 60, 2);
}

void WeatherStationClass::drawCurrentWeatherDetail()
{
    M5.Lcd.drawFastHLine(0, 20, 320, WHITE);
    M5.Lcd.drawFastVLine(100, 20, 210, WHITE);
    M5.Lcd.drawFastHLine(0, 230, 320, WHITE);
    M5.Lcd.drawJpg((uint8_t *)getMeteoconIconFromProgmem(conditions.weatherIcon),
                   sizeof(getMeteoconIconFromProgmem(conditions.weatherIcon)) / getMeteoconIconFromProgmem(conditions.weatherIcon)[0], 0, 25);
    M5.Lcd.setTextColor(WHITE);
    M5.Lcd.setFreeFont(&FreeSans9pt7b);
    M5.Lcd.drawCentreString("Current Conditions", 160, 2, 1);
    M5.Lcd.setTextColor(CYAN);
    M5.Lcd.setFreeFont(&FreeSans12pt7b);
    M5.Lcd.drawCentreString(conditions.weatherText, 220, 25, 1);
    ...
    M5.Lcd.setTextColor(YELLOW);
    M5.Lcd.drawCentreString("SunRise:", 45, 130, 1);
    M5.Lcd.drawCentreString("SunSet:", 45, 170, 1);
    M5.Lcd.setTextColor(WHITE);
    M5.Lcd.drawCentreString(astronomy.sunriseTime, 45, 150, 1);
    M5.Lcd.drawCentreString(astronomy.sunsetTime, 45, 190, 1);
}
...
void WeatherStationClass::drawLabelValue(uint8_t line, String label, String value)
{
    M5.Lcd.setFreeFont(&FreeSans9pt7b);
    M5.Lcd.setTextColor(ORANGE);
    M5.Lcd.drawString(label, labelX, 52 + line * 20, 1);
    M5.Lcd.setTextColor(WHITE);
    M5.Lcd.drawString(value, valueX, 52 + line * 20, 1);
}

void WeatherStationClass::drawForecastTable(uint8_t start)
{
    M5.Lcd.drawFastHLine(0, 20, 320, WHITE);
    M5.Lcd.drawFastVLine(60, 20, 210, WHITE);
    M5.Lcd.setTextColor(WHITE);
    M5.Lcd.setFreeFont(&FreeSans9pt7b);
    M5.Lcd.drawCentreString("Forecasts", 160, 2, 1);
    fy = 0;
    for (uint8_t i = start; i < start + 4; i++)
    {
        ...
        M5.Lcd.drawJpg((uint8_t *)getMiniMeteoconIconFromProgmem(forecasts[i].forecastIcon),
        ...
        M5.Lcd.setTextColor(ORANGE);
        M5.Lcd.setFreeFont(&FreeSans9pt7b);
        M5.Lcd.drawString(forecasts[i].forecastTitle, 70, (fy - 4), 1);
        M5.Lcd.setTextColor(WHITE);
        M5.Lcd.drawString(getShortText(forecasts[i].forecastIcon), 70, fy + 18, 1);
        M5.Lcd.setTextColor(WHITE);
        if (i % 2 == 0)
        {
        ...
        M5.Lcd.setFreeFont(&FreeSans9pt7b);
        M5.Lcd.drawRightString("Temp: " + temp + degreeSign, 315, (fy - 4), 1);
        M5.Lcd.setTextColor(BLUE);
        M5.Lcd.drawRightString("Precip: " + forecasts[i].PoP + "%", 315, fy + 18, 1);
        M5.Lcd.drawFastHLine(0, fy + 40, 320, WHITE);
    }
}
...
void WeatherStationClass::Run()
{
    ...
    if (WiFi.isConnected())
    {
        ...
        {
#else
        if (GetParams(SD_MMC, "/WeatherParams.txt"))
        {
#endif
            updateData(true);
        ...
        else
        {
            ...
            M5.Lcd.setTextColor(WHITE);
            M5.Lcd.setFreeFont(&FreeSans9pt7b);
            M5.Lcd.drawCentreString("WeatherParams.txt not found on SD!", 160, 60, 1);
            delay(3000);
            return;
        }
    }
    else
    {
        ...
        M5.Lcd.setTextColor(WHITE);
        M5.Lcd.setFreeFont(&FreeSans12pt7b);
        M5.Lcd.drawCentreString("Wifi Not Connected!", 160, 60, 1);
        delay(3000);
    }
}
